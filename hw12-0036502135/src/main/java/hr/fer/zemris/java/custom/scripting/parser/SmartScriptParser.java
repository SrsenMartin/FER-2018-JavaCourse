...
public class SmartScriptParser {
	 ...
	 */
	private void parseFor(Stack<Node> stack) {
		...
		List<Element> forCol = new ArrayList<>();
	 ...
	 */
	private ForLoopNode getForLoopNode(List<Element> forCol) {
		...
		if (forCol.size() == 3) {
			...
			startExpression = forCol.get(1);
			endExpression = forCol.get(2);
		} else if (forCol.size() == 4) {
			variable = (ElementVariable) forCol.get(0);
			startExpression = forCol.get(1);
			endExpression = forCol.get(2);
			stepExpression = forCol.get(3);
	 ...
	 */
	private void parseEnd(Stack<Node> stack) {
		SmartScriptToken current = lexer.nextToken();
	 ...
	 */
	private void parseEcho(Stack<Node> stack) {
		SmartScriptToken current = lexer.nextToken();

		List<Element> echoCol = new ArrayList<>();
