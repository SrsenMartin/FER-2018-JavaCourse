 ...
 * Inverts selected text letter case.
 * Action disabled if no text is selected.
 * Mnemonic key set to letter R.
 * Accelerator key: control + R.
 * 
 * @author Martin Sršen
 *
 */
public class InvertCaseAction extends LocalizableAction {
	 ...
	 */
	public InvertCaseAction(MultipleDocumentModel documentModel, String nameKey, String descKey, ILocalizationProvider lp) {
		super(nameKey, descKey, lp);
		this.documentModel = documentModel;
		Util.createSingleAction(this, "control R", KeyEvent.VK_R);
	}

	 /**
	  * Called every time actions is called.
	  * Calls method that will invert selected text letter case.
	  * 
	  * @param event	Details about occurred event.
	  */
	@Override
	public void actionPerformed(ActionEvent event) {
		Util.executeTextAction(documentModel, SelectedTextAction.INVERT);
	}
}
