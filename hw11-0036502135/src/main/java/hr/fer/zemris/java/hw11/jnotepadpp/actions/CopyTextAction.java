...
public class CopyTextAction extends LocalizableAction {
	 ...
	 */
	public CopyTextAction(MultipleDocumentModel documentModel, String nameKey, String descKey, ILocalizationProvider lp) {
		super(nameKey, descKey, lp);
		this.documentModel = documentModel;
		Util.createSingleAction(this, "control C", KeyEvent.VK_C);
		setEnabled(false);
	}

	 /**
	  * Called every time actions is called.
	  * Calls method that will copy selected text into system clipboard.
	  * 
	  * @param event	Details about occurred event.
	  */
	@Override
	public void actionPerformed(ActionEvent event) {
		Util.executeTextAction(documentModel, SelectedTextAction.COPY);
	}
}
